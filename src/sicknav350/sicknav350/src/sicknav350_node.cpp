...
void PublishLaserTransform(tf::TransformBroadcaster laser_broadcaster, std::string header_frame_id, std::string child_frame_id)
{
    ...
    //		    tf::StampedTransform(tf::Transform(tf::Quaternion(0, 0, 0, 1), tf::Vector3(0.1, 0, 0.5 )),
...
int main(int argc, char *argv[])
{
    ...
    nh_ns.param<bool>("publish_tf", publish_tf_, false);
    ...
    nh_ns.param<std::string>("frame_id", frame_id, "laser");           //laser frame for scan data
    nh_ns.param<std::string>("fixed_frame_id", fixed_frame_id, "map"); // nav350 mount position frame on the robot

    nh_ns.param<std::string>("laser_frame_id", laser_frame_id, "laser_frame");           //global cooridnate frame measurement for navigation and position based on reflectors
    ...
    nh_ns.param("scan_rate", sick_motor_speed, 8);
